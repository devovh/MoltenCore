...
ABModuleScript::ABModuleScript(const char* name)
    : ModuleScript(name)
{
    ScriptRegistry<ABModuleScript>AddScript();
}*/
                    ...
                    if (!playerList.IsEmpty())
                    {
                        for (Map::PlayerList::const_iterator playerIteration = playerList.begin(); playerIteration != playerList.end(); ++playerIteration)
                        {
                            if (Player* playerHandle = playerIteration->GetSource())
                            {
                                ...
                                chatHandle.PSendSysMessage("|cffFF0000 [AutoBalance]|r|cffFF8000 {} entered the Instance {}. Auto setting player count to {} (Player Difficulty Offset = {}) |r", player->GetName().c_str(), map->GetMapName(), mapABInfo->playerCount + PlayerCountDifficultyOffset, PlayerCountDifficultyOffset);
                            }
                        }
                    }
                }
                ...
                if (AutoBalanceCheck)
                {
                    for (Map::PlayerList::const_iterator itr = pl.begin(); itr != pl.end(); ++itr)
                    {
                        if (Player* plr = itr->GetSource())
                        {
                            ...
                            Warhead::Text::SendNotification(plr->GetSession(), "|cff4cff00%s|rAccess can be unlocked during non-combat", map->GetMapName());
                            ...
                            Warhead::Text::SendNotification(plr->GetSession(), "|cffffffff[%s]|rThe player left during the battle|cff4cff00%s|rInstance elastic lock", player->GetName().c_str(), map->GetMapName());
                        }
                    }
                }
                else
                ...
                if (map->GetEntry()->IsDungeon() && player)
                {
                    ...
                    if (!playerList.IsEmpty())
                    {
                        for (Map::PlayerList::const_iterator playerIteration = playerList.begin(); playerIteration != playerList.end(); ++playerIteration)
                        {
                            ...
                            {
                                ChatHandler chatHandle = ChatHandler(playerHandle->GetSession());
                                chatHandle.PSendSysMessage("|cffFF0000 [-AutoBalance]|r|cffFF8000 {} left the Instance {}. Auto setting player count to {} (Player Difficulty Offset = {}) |r", player->GetName().c_str(), map->GetMapName(), mapABInfo->playerCount, PlayerCountDifficultyOffset);
                            }
                        }
                    }
                }
