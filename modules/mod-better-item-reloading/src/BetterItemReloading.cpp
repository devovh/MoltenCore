...
public:
    ...
    static bool HandleBetterReloadItemsCommand(ChatHandler* handler, const char* args)
    {
        ...
        //for (Tokenizer::const_iterator itr = entries.begin(); itr != entries.end(); ++itr)
        {
            ...
            itemTemplate->MaxCount = fields[24].Get<int16>();
            ...
            for (uint8 i = 0; i < itemTemplate->StatsCount; ++i)
            {
                ...
                itemTemplate->ItemStat[i].ItemStatValue = int32(fields[29 + i * 2].Get<uint8>());
            }
            ...
            {
                //sLog->UsingDefaultLogs("Item (Entry: %u) has wrong Class value (%u)", entry, itemTemplate->Class);
...
private:
    static void SendItemQuery(Player* player, ItemTemplate* item)
    {
        ...
        if (loc_idx >= 0)
        {
            if (ItemLocale const* il = sGameLocale->GetItemLocale(item->ItemId))
            {
