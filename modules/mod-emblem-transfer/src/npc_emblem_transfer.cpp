...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action)
    {
        ...
        if (action == ACTION_RETRIEVE_EMBLEMS)
        {
            ...
            if (result)
            {
                do
                {
                    ...
                    {
                        Warhead::Text::SendNotification(player->GetSession(), LANG_ITEM_CANNOT_CREATE, emblemId, noSpaceForCount);
                        continue;
                    }
                ...
                Warhead::Text::SendNotification(player->GetSession(), "Thank you for using the emblem transfer service!");
        ...
        if (sender == GOSSIP_SENDER_MAIN)
        {
            ...
            {
                Warhead::Text::SendNotification(player->GetSession(), "You don't have enough emblems! The minimum amount is {}", minAmount);
                return OnGossipSelect(player, creature, sender, ACTION_CLOSE);
            }
    ...
    bool OnGossipSelectCode(Player* player, Creature* creature, uint32 sender, uint32 action, const char* code)
    {
        if (!isNumber(code))
        {
            Warhead::Text::SendNotification(player->GetSession(), "Please enter a valid number!");
            return OnGossipSelect(player, creature, sender, ACTION_CLOSE);
        }
        ...
        if (emblemId == 0)
        {
            Warhead::Text::SendNotification(player->GetSession(), "There was a problem processing your request. Please notify an administrator.");
            return OnGossipSelect(player, creature, sender, ACTION_CLOSE);
        }
        ...
        {
            Warhead::Text::SendNotification(player->GetSession(), "You don't have enough emblems!");
            return OnGossipSelect(player, creature, sender, ACTION_CLOSE);
        }
