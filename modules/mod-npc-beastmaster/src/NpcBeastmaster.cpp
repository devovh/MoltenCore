...
void NpcBeastmaster::ShowMainMenu(Player* player, Creature* creature)
{
    ...
    {
        std::string messageExperience = Warhead::StringFormat("Sorry {}, but you must reach level {} before adopting a pet.", player->GetName(), BeastMasterMinLevel);
...
void NpcBeastmaster::CreatePet(Player* player, Creature* creature, uint32 action)
{
    ...
    // Farewell
    std::string messageAdopt = Warhead::StringFormat("A fine choice {}! Take good care of your {} and you will never face your enemies alone.", player->GetName(), pet->GetName());
...
void NpcBeastmaster::LoadPets(std::string pets, PetsStore& petMap)
{
    ...
    while (std::getline(petsStringStream, delimitedValue, ','))
    {
        if (count % 2 == 0)
            ...
            petMap[petName] = *Warhead::StringTo<uint32>(delimitedValue);
